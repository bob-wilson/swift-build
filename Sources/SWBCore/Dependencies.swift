...
public struct LinkDependenciesContext: Sendable, SerializableCodable {
    ...
    public var linkDependencies: [String]

    public init(validate: BooleanWarningLevel, linkDependencies: [String]) {
        self.validate = validate
        self.linkDependencies = linkDependencies
    }

    public init?(settings: Settings) {
        let validate = settings.globalScope.evaluate(BuiltinMacros.VALIDATE_LINK_DEPENDENCIES)
        guard validate != .no else { return nil }
        // FIXME: get linkDependencies from OTHER_LDFLAGS and the link phase
        self.init(validate: validate, linkDependencies: [])
    }
    ...
    public func makeDiagnostics(files: [Path]) -> [Diagnostic] {
        ...
        func findLibraryName(_ file: Path) -> String? {
        ...
        let missingDeps = files.filter { file in
            // FIXME: if autolinking is enabled, exclude all the transitive module libraries
            guard let name = findFrameworkName(file) ?? findLibraryName(file) else {
                return false
            }
            return !linkDependencies.contains(name)
        }
...
//        let fixIt = fixItContext?.makeFixIt(newLinks: missingDeps)
//        let fixIts = fixIt.map { [$0] } ?? []
//
//        let message = "Missing entries in \(BuiltinMacros.OTHER_LDFLAGS) for link dependencies (\($0.name): \(missingDeps.map { $0.asBuildSettingEntryQuotedIfNeeded }.sorted().joined(separator: " "))"
//
//        let location: Diagnostic.Location = fixIt.map {
//            Diagnostic.Location.path($0.sourceRange.path, line: $0.sourceRange.endLine, column: $0.sourceRange.endColumn)
//        } ?? Diagnostic.Location.buildSetting(BuiltinMacros.OTHER_LDFLAGS)
//
//        return [Diagnostic(
//            behavior: behavior,
//            location: location,
...
//            fixIts: fixIts)]
    ...
    func signatureData() -> String {
        return "validate:\(validate),links:\(linkDependencies.joined(separator: ":"))"
    }
}
